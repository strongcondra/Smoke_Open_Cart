...
class ModelPavblogcategory extends Model {
    ...
    public function genTree( $parent, $level , $class="", $category_id ){
        if( $this->hasChild($parent) ){
            ...
            $output .='<li style="display:block;padding-top: 13px;">';
				    $output .= '<a style="color:#95C120;font-size:14px;font-weight: 710;" href="'.$this->url->link('pavblog/category',"blogcategory_id=".$menu['category_id']).'" title="'.$menu['title'].'">'.$menu['title']."</a>";
					    $output .= '</li>';
                ...
                $sql = "SELECT * from ".DB_PREFIX."pavblog_blog as af left join ".DB_PREFIX."pavblog_blog_description pd on af.blog_id=pd.blog_id where af.category_id =".$menu['category_id'];
                   $sql .= ' ORDER BY `position`  ';
				  $query = $this->db->query( $sql );
				   foreach($query->rows as $blogg){
					 
					     $output .='<li style="display:block;">';
			      $output .= '<a  style="padding-left:16px; display: inline-block; color:#000;font-size:14px;" href="'.$this->url->link('pavblog/blog',"blog_id=".$blogg['blog_id']).'" title2="'.$blogg['title'].'">'.$blogg['title']."</a>";
